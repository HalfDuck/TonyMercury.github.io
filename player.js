 ...
 * QQ: 19124152
 * Time: 2020/02/13
 * Disclaimer: 插件修改于明月浩空免费版，仅用于学习交流。
 * 本程序禁止商业传播，如发现将禁止软件的免费使用
 */
...
var lzxPlayerInit = function () {
    ...
    $songFrom.html('<a style="color:#f00">music.yiweibang.com</a>');
    ...
    var lzxLrc = {
        load: function () {
            ...
            setTimeout(function () {
                ...
                $('.switch-ksclrc').show();
                $.ajax({
                    url: webURL + "/api/info/musicLyric?songId=" + songSheetList[albumId].songIds[songId] + "&type="
        ...
        lrc: {
            format: function (cont) {
                ...
                for (var i = 1; i < lrcCont.length; i += 2) {
                    ...
                    if (i == 1) {
                        lrcLine += '<li class="lzxLrc' + timer + ' current" style="color:rgba(0,255,0)">' + lrcCont[i + 1] + '</li>' // 备份原rgba rgba(' + mainColor + ',1)
                    } else {
                        lrcLine += '<li class="lzxLrc' + timer + '"  style="color:rgba(' + mainColor + ',1)" >' + lrcCont[i + 1] + '</li>' // 原rgba为空
                    }
                }
            ...
            play: function () {
                ...
                if ($.inArray(timeNow, lrcTimeLine) > 0) {
                    ...
                    if (!$lineNow.hasClass(cur)) {
                        $lineNow.css('color','rgba(0,255,0)'); // 备份原rgba rgba(' + mainColor + ',1)
                        $lineNow.addClass(cur).siblings().removeClass(cur).css('color','rgba(' + mainColor + ',1)'); // 原rgba为空
                        $('#lzxLrc').animate({
    ...
    //获取歌单列表数据
    $.ajax({
        url: webURL + '/api/info/info?id=' + keyId,
        type: 'GET',
        dataType: 'script',
        success: function () {
            if(playerWidth !== -1){
    ...
    function netmusic() {
        audio.src = webURL + "/api/info/musicUrl?songId=" + songSheetList[albumId].songIds[songId]+"&type="+songSheetList[albumId].songTypes[songId];
        $('.switch-down').show();
